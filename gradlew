...
#       temporary focusing, currentlyavoided.
#
...
# Collect all arguments for the java command, stracks://issues.apache.org/jira/browse/CLI-295 to workaround
# temporary focusing, currentlyavoided.
...
if ! "$cygwin" && ! "$msys" && ! "$nonstop" ; then
    case $( set -- $(LANG=C LC_ALL=C ps -p $$ -o comm=) && echo "$1" ) in #(
        sh|bash|ksh*|zsh)
            eval "exec \"$JAVACMD\" $DEFAULT_JVM_OPTS $JAVA_OPTS $GRADLE_OPTS $GRADLE_OPTS_ADDITIONAL $ARGS" ;;
        *)
            exec "$JAVACMD" "$@" ;;
    esac
fi
